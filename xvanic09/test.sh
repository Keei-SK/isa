...
test_standard() { #$1 - name, $2 - server
    ...
    ./dns -r -s $2 $1 | grep $1 | cut -d "," -f 5 | grep "\S" | sed -e 's/^ *//g' > tests/$1.out 2>/dev/null
...
test_ipv6_standard() { #$1 - name, $2 - server
    ...
    ./dns -r -6 -s $2 $1 | grep $1 | cut -d "," -f 5 | grep "\S" | sed -e 's/^ *//g' > tests/$1.out 2>/dev/null
    diff -c tests/$1.out tests/$1.ref > /dev/null 2>/dev/null
    result=$?
    rm -rf tests/$1.*
    printf $1
...
test_reverse() { #$1 - name, $2 - server
    ...
    ./dns -r -x -s $2 $1 | grep $1 | cut -d "," -f 5 | grep "\S" | sed -e 's/^ *//g' > tests/$1.out 2>/dev/null
    diff -c tests/$1.out tests/$1.ref > /dev/null 2>/dev/null
    result=$?
    #rm -rf tests/$1.*
    printf $1
...
test_standard "merlin.fit.vut.cz" $SERVER

test_ipv6_standard "fit.vut.cz" $SERVER
test_ipv6_standard "merlin.fit.vutbr.cz" $SERVER
test_ipv6_standard "merlin.fit.vut.cz" $SERVER
